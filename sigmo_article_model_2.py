...
#from lib_random_walk import random_walk
from no_memory_random_walk_cython import random_walk

#PARAMETERS

M = 10000 #mémoire du système
time = 2000000
...
##    delta_gamma = 10 ** delta_gamma_power
    ...
    delta_gamma = np.random.uniform(delta_min,delta_max)
    ...
    if (db < tol_c)&(dder < tol_c):

        #Marche aléatoire

        N_W = random_walk(gamma,beta,time,W,M,M_W,alloc,both_mechs)
...
##                    total_time_data[j_loop] = len(N_W)
                    ...
                    if plot_logit == 1: 
                        ...
                        plt.bar(range(starting_point,selected_pair[0]),N_W[starting_point:selected_pair[0]],color='r')
                        plt.bar(range(selected_pair[0],min(selected_pair[1]+1,len(N_W))),N_W[selected_pair[0]:min(selected_pair[1]+1,len(N_W))],color='b')
                        plt.show()
...
plt.title('Time of growth phase vs delta parameter')
plt.show()
...
lambdaA=trA/2.*(1-math.sqrt(1-4.*detA/(trA**2)))
ob=mcov[0,1]
oa=mcov[0,0]
oc=mcov[1,1]

coeff_P=math.sqrt(ob**2/(ob**2+(lambdaA-oa)**2))
